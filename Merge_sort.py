...
def merge(lst,l,m,r,drawrectangle,delay):
    ...
    while i<n1 and j<n2:
        if L[i] <= R[j]:
            lst[k] = L[i]
            drawrectangle(lst, ['blue' if x == j else 'green' if x == k else 'red' for x in range(len(lst))] )
            time.sleep(delay)
            i+=1
        else:
            lst[k] = R[j]
    ...
    while i<n1:
        lst[k] = L[i]
        drawrectangle(lst, ['blue' if x == i else 'green' if x == k else 'red' for x in range(len(lst))] )
        time.sleep(delay)
        i+=1
        k+=1
